...
//            $jobs['total'] = $totaljobs;
    ...
    public function store($user, $data)
    {
        ...
        if ($user->user_type == GlobalConstants::CUSTOMER_ROLE_ID) {
            $cuser = $user;
...
//            $this->sendNotificationToSuitableTranslators($job->id, $data, '*');// send Push for New job posting
    ...
    public function storeJobEmail($data)
    {
        ...
        $response['job'] = $job;
        $response['status'] = 'success';
        $data = GlobalHelper::jobToData($job);
        Event::fire(new JobWasCreated($job, $data, '*'));
        return $response;
...
//        if (in_array($data['status'], ['pending', 'assigned']) && date('Y-m-d H:i:s') <= $job->due) {
        ...
        if ($data['status'] == 'pending') {
            ...
            $job->save();
            $job_data = GlobalHelper::jobToData($job);

            $subject = 'Vi har nu återöppnat er bokning av ' . TeHelper::fetchLanguageFromJobId($job->from_language_id) . 'tolk för bokning #' . $job->id;
            $this->mailer->send($email, $name, $subject, 'emails.job-change-status-to-customer', $dataEmail);

            GlobalHelper::sendNotificationTranslator($job, $job_data, '*');   // send Push all sutiable translators
...
//        if (in_array($data['status'], ['withdrawnbefore24', 'withdrawafter24', 'timedout', 'assigned'])) {
        ...
        if ($data['status'] == 'assigned' && $changedTranslator) {
            ...
            GlobalHelper::sendSessionStartRemindNotification($user, $job, $language, $job->due, $job->duration);
            GlobalHelper::sendSessionStartRemindNotification($translator, $job, $language, $job->due, $job->duration);
            return true;
        } else {
...
//        }
    ...
    public function sendExpiredNotification($job, $user)
    {
        ...
        if (GlobalHelper::isNeedToSendPush($user->id)) {
            ...
            GlobalHelper::sendPushNotificationToSpecificUsers($users_array, $job->id, $data, $msg_text, $this->isNeedToDelayPush($user->id));
        }
    }
    ...
    public function acceptJobWithId($job_id, $cuser)
    {
        ...
        if (!Job::isTranslatorAlreadyBooked($job_id, $cuser->id, $job->due)) {
            if ($job->status == 'pending' && Job::insertTranslatorJobRel($cuser->id, $job_id)) {
                ...
                if (GlobalHelper::isNeedToSendPush($user->id)) {
                    $users_array = array($user);
                    GlobalHelper::sendPushNotificationToSpecificUsers($users_array, $job_id, $data, $msg_text, $this->isNeedToDelayPush($user->id));
                }
        ...
        */
        ...
        if ($cuser->is('customer')) {
            ...
            if ($translator) {
                ...
                if (GlobalHelper::isNeedToSendPush($translator->id)) {
                    $users_array = array($translator);
                    GlobalHelper::sendPushNotificationToSpecificUsers($users_array, $job_id, $data, $msg_text, $this->isNeedToDelayPush($translator->id));// send Session Cancel Push to Translaotor
                }
            }
        } else {
            if ($job->due->diffInHours(Carbon::now()) > 24) {
                ...
                if ($customer) {
                    ...
                    if (GlobalHelper::isNeedToSendPush($customer->id)) {
                        $users_array = array($customer);
                        GlobalHelper::sendPushNotificationToSpecificUsers($users_array, $job_id, $data, $msg_text, $this->isNeedToDelayPush($customer->id));     // send Session Cancel Push to customer
                    }
                }
                $job->status = 'pending';
                $job->created_at = date('Y-m-d H:i:s');
...
//                Event::fire(new JobWasCanceled($job));
                ...
                GlobalHelper::sendNotificationTranslator($job, $data, $translator->id);   // send Push all sutiable translators
                $response['status'] = 'success';
            } else {
                $response['status'] = 'fail';
...
//        $jobs = TeHelper::convertJobIdsInObjs($job_ids);
    ...
    public function getAll(Request $request, $limit = null)
    {
        ...
        if ($cuser && $cuser->user_type == GlobalConstants::SUPERADMIN_ROLE_ID) {
            $allJobs = Job::query();
...
//        $this->logger->addInfo('USER #' . Auth::user()->id . ' reopen booking #: ' . $jobid);
        ...
        if (isset($affectedRows)) {
            GlobalHelper::sendNotificationByAdminCancelJob($new_jobid);
